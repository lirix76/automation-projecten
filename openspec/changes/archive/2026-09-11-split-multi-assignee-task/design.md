...
- No trigger's `filtersObj`/`recordMatchesConditions` condition can test a linked-record field's array length (only `isEmpty`/`isNotEmpty`/`hasAnyOf` are used anywhere in this base, and the automation-tooling docs explicitly warn against `length()` there). A `conditionalGroup` node's condition tree turned out to be different: built directly in the Airtable UI (not through the MCP `create_automation` tool, which produced the same warning), it accepts `length({"$ref":...})` against a numeric literal just fine — see the added conditionalGroup gate under Decisions below.
...
- Via MCP (`create_automation`): the `recordCreated` trigger on the Taken table, plus a placeholder `updateRecord` node (logs a warning to `systemMessages`) standing in for the script node, since an automation must have at least one node and every node type that could realistically stand in for a script (`customScript`, `noOp`) is itself rejected as `readOnlyNodeType`.
- Manual, by the user: replacing the placeholder node in the Airtable UI with a "Run a script" action and pasting in the script this change produces, per the `airtable-automations` skill's script conventions (field IDs only, `systemMessages` prepend pattern, header comment with the automation's link).

**Added after deployment, by the user directly in the Airtable UI: a `conditionalGroup` node wrapping the script, gated on `length(Toewijzen aan) > 1`.** Not part of the original design — the trigger stays `recordCreated` (unchanged), so this doesn't reopen the "only splits at creation" decision above. It's a refinement of the "split logic entirely inside the script" decision: instead of the script running unconditionally and returning early for single-assignee records, the count check now also gates whether the script runs at all. Cheaper (no wasted script execution for the common case) and makes the skip visible in Airtable's automation run history. The script's own internal `length <= 1` guard is now redundant in normal operation but was left in place as a fallback. Notably, this length() check works fine inside a `conditionalGroup` condition tree when built directly in the Airtable UI, contradicting the MCP tooling's own warning against `length()` in condition operands (see Context) — that restriction turned out to be about what the MCP's automation-building tool can reliably render, not a hard platform limitation.
